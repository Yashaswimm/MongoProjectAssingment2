...
public class PaymentService {
    ...
    public Payment editPayment(String id, Paymentdto paymentdto) {
        ...
        if (optionalPayment.isPresent()) {
            ...
            payment.setVendor(paymentdto.getVendor());
    ...
    // 7.Get NetAmount By invoice number

    public Double getNetAmountByInvoiceNumber(String invoiceNumber) {
        Payment payment = paymentRepository.findByInvoicenumber(invoiceNumber);
        if(payment==null)
        return 0.0;
        else
        return payment.getAmount()-(((double)payment.getTds()/100.0)*payment.getAmount());
    }
